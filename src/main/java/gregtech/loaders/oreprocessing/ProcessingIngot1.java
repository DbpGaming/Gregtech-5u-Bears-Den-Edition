...
public class ProcessingIngot1 implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        ...
        if (!aMaterial.contains(SubTag.NO_WORKING))
            RECIPE_ADDER_INSTANCE.addLatheRecipe(GT_Utility.copyAmount(1L, aStack), GT_OreDictUnificator.get(OrePrefixes.stick, aMaterial, 1L), GT_OreDictUnificator.get(OrePrefixes.dustSmall, aMaterial.getMaceratingInto(), 2L), (int) Math.max(aMaterial.getMass() * 5L, 1L), 16);
        if (!aMaterial.contains(SubTag.NO_SMELTING)) {
            ...
            if ((GT_ModHandler.getSmeltingOutput(aStack, false, null) == null) && (GT_OreDictUnificator.get(OrePrefixes.nugget, aMaterial.getSmeltingInto(), 1L) != null) && (!GT_ModHandler.addSmeltingRecipe(aStack, GT_OreDictUnificator.get(OrePrefixes.nugget, aMaterial.getSmeltingInto(), 9L))))
                GT_ModHandler.addShapelessCraftingRecipe(GT_OreDictUnificator.get(OrePrefixes.nugget, aMaterial.getSmeltingInto(), 9L), new Object[]{aOreDictName});
        }
        ItemStack tStack;
        if ((null != (tStack = GT_OreDictUnificator.get(OrePrefixes.dust, aMaterial.getMaceratingInto(), 1L))) && (
                (aMaterial.isBlastFurnaceRequired()) || (aMaterial.contains(SubTag.NO_SMELTING)))) {
        ...
        if (aMaterial == Materials.get("Mercury")) {
