...
public class ProcessingCrushedPurified implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        GT_ModHandler.addThermalCentrifugeRecipe(GT_Utility.copyAmount(1L, aStack), (int) Math.min(5000L, Math.abs(aMaterial.getMass() * 20L)), GT_OreDictUnificator.get(OrePrefixes.crushedCentrifuged, aMaterial.getMaceratingInto(), GT_OreDictUnificator.get(OrePrefixes.dust, aMaterial.getMaceratingInto(), 1L), 1L), GT_OreDictUnificator.get(OrePrefixes.dustTiny, GT_Utility.selectItemInList(1, aMaterial.getMaceratingInto(), aMaterial.getOreByProducts()), 1L));
