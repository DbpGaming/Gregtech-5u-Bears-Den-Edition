...
public class GT_Cover_AirVent
        ...
        if (aSide != 6) {
            ...
            if ((aCoverVariable < 3) && (aTileEntity instanceof IFluidHandler)
                    && (tBlock == Blocks.air)) {
                ((IFluidHandler) aTileEntity).fill(ForgeDirection.getOrientation(aSide), Materials.get("Air").getGas(1000), true);
            }
        }
        return aCoverVariable;
    }

        public boolean letsFluidIn(byte aSide, int aCoverID, int aCoverVariable, Fluid aFluid, gregtech.api.interfaces.tileentity.ICoverable
        aTileEntity) {
