		...
		if (aSide != 6) {
			...
			if ((aCoverVariable < 3) && ((aTileEntity instanceof IFluidHandler))) {
				if ((aSide == 1) &&
					...
					if (tAmount > 0) {
						((IFluidHandler) aTileEntity).fill(ForgeDirection.getOrientation(aSide), Materials.get("Water").getFluid(aTileEntity.getWorld().isThundering() ? tAmount * 2 : tAmount), true);
					}
				}
				FluidStack tLiquid = null;
				if (tBlock != null) {
					if (((tBlock == Blocks.water) || (tBlock == Blocks.flowing_water)) && (aTileEntity.getMetaIDAtSide(aSide) == 0)) {
						tLiquid = Materials.get("Water").getFluid(1000L);
					} else if (((tBlock == Blocks.lava) || (tBlock == Blocks.flowing_lava)) && (aTileEntity.getMetaIDAtSide(aSide) == 0)) {
						tLiquid = Materials.get("Lava").getFluid(1000L);
