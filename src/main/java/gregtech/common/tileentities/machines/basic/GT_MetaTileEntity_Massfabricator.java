...
public class GT_MetaTileEntity_Massfabricator
    ...
    public void onConfigLoad(GT_Config aConfig) {
        ...
        Materials.get("UUAmplifier").setToolTip("Mass Fabricator Eff/Speed Bonus: x" + sUUASpeedBonus);
    }

    public int checkRecipe() {
        FluidStack tFluid = getDrainableStack();
        if ((tFluid == null) || (tFluid.amount < getCapacity())) {
            this.mOutputFluid = Materials.get("UUMatter").getFluid(1L);
            this.mEUt = ((int) TIERED_VOLTAGES[this.mTier]);
            this.mMaxProgresstime = (sDurationMultiplier / (1 << this.mTier - 1));
            if (((tFluid = getFillableStack()) != null) && (tFluid.amount >= sUUAperUUM) && (tFluid.isFluidEqual(Materials.get("UUAmplifier").getFluid(1L)))) {
    ...
    public boolean isFluidInputAllowed(FluidStack aFluid) {
        return aFluid.isFluidEqual(Materials.get("UUAmplifier").getFluid(1L));
    }
