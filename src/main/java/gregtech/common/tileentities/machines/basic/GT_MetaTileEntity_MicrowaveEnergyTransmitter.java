...
public class GT_MetaTileEntity_MicrowaveEnergyTransmitter extends GT_MetaTileEntity_BasicTank {
    ...
    public GT_MetaTileEntity_MicrowaveEnergyTransmitter(int aID, String aName, String aNameRegional, int aTier) {
        super(aID, aName, aNameRegional, aTier, 3, "Transmits Energy Wirelessly");
    }
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            if ((getBaseMetaTileEntity().isAllowedToWork()) && (getBaseMetaTileEntity().getRedstone())) {
                if (getBaseMetaTileEntity().getStoredEU() > (TIERED_VOLTAGES[mTier] * 16)) {
                    ...
                    long tEnergyTrans = Math.min(TIERED_VOLTAGES[mTier], getBaseMetaTileEntity().getStoredEU());
                    if (tTile != null && tTile instanceof IEnergyConnected) {
                        if (((IEnergyConnected) tTile).injectEnergyUnits((byte) 6, TIERED_VOLTAGES[mTier], 1) > 0) {
                            int tLoss = 1;
                            if (mMaxLossDistance != 0) {
                                tLoss = 10 + tDistance * (mMaxLoss - 10) / mMaxLossDistance;
                            }
                            getBaseMetaTileEntity().decreaseStoredEnergyUnits(TIERED_VOLTAGES[mTier] + ((TIERED_VOLTAGES[mTier] * tLoss) / 100), false);
                        }
                    }
                }
    ...
    public long maxEUStore() {
        return TIERED_VOLTAGES[mTier] * 256;
    }
