...
public class GT_MetaTileEntity_Scanner
    ...
    public int checkRecipe() {
        ...
        if (getOutputAt(0) != null) {
            ...
            if ((getFillableStack() != null) && (getFillableStack().containsFluid(Materials.get("Honey").getFluid(100L)))) {
                try {
            ...
            if (ItemList.Tool_DataOrb.isStackEqual(getSpecialSlot(), false, true)) {
                ...
                if ((tData != null) && ((tData.mPrefix == OrePrefixes.dust) || (tData.mPrefix == OrePrefixes.cell)) && (tData.mMaterial.mMaterial.getElement() != null) && (!tData.mMaterial.mMaterial.getElement().mIsIsotope) && (tData.mMaterial.mMaterial != Materials.get("Magic")) && (tData.mMaterial.mMaterial.getMass() > 0L)) {
                    getSpecialSlot().stackSize -= 1;
                    aStack.stackSize -= 1;

                    this.mOutputItems[0] = ItemList.Tool_DataOrb.get(1L);
                    Behaviour_DataOrb.setDataTitle(this.mOutputItems[0], "Elemental-Scan");
                    Behaviour_DataOrb.setDataName(this.mOutputItems[0], tData.mMaterial.mMaterial.getElement().name());
