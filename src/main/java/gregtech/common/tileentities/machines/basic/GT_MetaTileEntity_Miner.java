...
public class GT_MetaTileEntity_Miner extends GT_MetaTileEntity_BasicMachine {
    ...
    final static int[] RADIUS = new int[]{4, 8, 16, 32, 48}; //Miner radius per tier
    final static int[] SPEED = new int[]{20, 20, 20, 20, 20}; //Miner cycle time per tier
    final static int[] ENERGY = new int[]{8, 16, 64, 256, 1024}; //Miner energy consumption per tier
    ...
    public IMetaTileEntity newMetaEntity(IGregTechTileEntity aTileEntity) {
        return new GT_MetaTileEntity_Miner(mName, mTier, mDescription, mTextures, mGUIName, mNEIName);
    }
    ...
    public String[] getDescription() {
        return new String[]{
                ...
                "Quarries in a " + (RADIUS[mTier]) + " block radius",
                "Returns pipes at bedrock"};
    }
    ...
    public long maxEUStore() {
        return mTier == 1 ? 4096 : ENERGY[mTier] * 64;
    }
