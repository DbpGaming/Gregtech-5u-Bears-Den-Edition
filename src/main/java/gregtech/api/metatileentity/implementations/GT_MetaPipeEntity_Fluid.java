...
public class GT_MetaPipeEntity_Fluid extends MetaPipeEntity {
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aConnections, byte aColorIndex, boolean aConnected, boolean aRedstone) {
    	...
    	if (mDisableInput == 0) return new ITexture[]{aConnected ? getBaseTexture(tThickNess, mPipeAmount, mMaterial, aColorIndex) : new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipe.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
        byte tMask = 0;
        ...
        return new ITexture[]{aConnected ? getBaseTexture(tThickNess, mPipeAmount, mMaterial, aColorIndex) : new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipe.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), getRestrictorTexture(tMask)};
    }

    protected static final ITexture getBaseTexture(float aThickNess, int aPipeAmount, Materials aMaterial, byte aColorIndex) {
    	if (aPipeAmount >= 9) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeNonuple.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	if (aPipeAmount >= 4) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeQuadruple.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	if (aThickNess < 0.124F) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipe.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	if (aThickNess < 0.374F) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeTiny.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	if (aThickNess < 0.499F) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeSmall.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	if (aThickNess < 0.749F) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeMedium.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	if (aThickNess < 0.874F) return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeLarge.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    	return new GT_RenderedTexture(aMaterial.getTextureSet().mTextures[OrePrefixes.pipeHuge.mTextureIndex], Dyes.getModulation(aColorIndex, aMaterial.getRGBa()));
    }
    	...
    	if (GT_Mod.gregtechproxy.gt6Pipe) {
    		...
    		if (aPlayer.isSneaking()) {
    			if (isInputDisabledAtSide(tSide)) {
    				mDisableInput &= ~tMask;
    				GT_Utility.sendChatToPlayer(aPlayer, "Input enabled");
    				if (!isConnectedAtSide(tSide))
            			connect(tSide);
                        } else {
    				mDisableInput |= tMask;
    				GT_Utility.sendChatToPlayer(aPlayer, "Input disabled");
    			}
    		} else {
    			if (!isConnectedAtSide(tSide)) {
    				if (connect(tSide) > 0)
