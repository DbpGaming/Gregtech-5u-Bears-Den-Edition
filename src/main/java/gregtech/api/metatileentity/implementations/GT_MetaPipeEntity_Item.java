...
public class GT_MetaPipeEntity_Item extends MetaPipeEntity implements IMetaTileEntityItemPipe {
    ...
    public byte getTileEntityBaseType() {
        return mMaterial == null ? 4 : (byte) ((mMaterial.contains(SubTag.WOOD) ? 12 : 4) + Math.max(0, Math.min(3, mMaterial.getToolQuality())));
    }
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aConnections, byte aColorIndex, boolean aConnected, boolean aRedstone) {
        if (mIsRestrictive) {
            if (aConnected) {
                ...
                if (tThickNess < 0.37F)
                    return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeTiny.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), new GT_RenderedTexture(Textures.BlockIcons.PIPE_RESTRICTOR)};
                if (tThickNess < 0.49F)
                    return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeSmall.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), new GT_RenderedTexture(Textures.BlockIcons.PIPE_RESTRICTOR)};
                if (tThickNess < 0.74F)
                    return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeMedium.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), new GT_RenderedTexture(Textures.BlockIcons.PIPE_RESTRICTOR)};
                if (tThickNess < 0.99F)
                    return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeLarge.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), new GT_RenderedTexture(Textures.BlockIcons.PIPE_RESTRICTOR)};
                return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeHuge.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), new GT_RenderedTexture(Textures.BlockIcons.PIPE_RESTRICTOR)};
            }
            return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipe.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa())), new GT_RenderedTexture(Textures.BlockIcons.PIPE_RESTRICTOR)};
        }
        if (aConnected) {
            float tThickNess = getThickNess();
            if (tThickNess < 0.37F)
                return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeTiny.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
            if (tThickNess < 0.49F)
                return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeSmall.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
            if (tThickNess < 0.74F)
                return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeMedium.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
            if (tThickNess < 0.99F)
                return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeLarge.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
            return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipeHuge.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
        }
        return new ITexture[]{new GT_RenderedTexture(mMaterial.getTextureSet().mTextures[OrePrefixes.pipe.mTextureIndex], Dyes.getModulation(aColorIndex, mMaterial.getRGBa()))};
    }
    ...
    public boolean onWrenchRightClick(byte aSide, byte aWrenchingSide, EntityPlayer aPlayer, float aX, float aY, float aZ) {
    	if (GT_Mod.gregtechproxy.gt6Pipe) {
    		...
    		if (!isConnectedAtSide(tSide)) {
    			if (connect(tSide) > 0)
    				GT_Utility.sendChatToPlayer(aPlayer, "Connected");
    		}
    		else {
    			disconnect(tSide);
